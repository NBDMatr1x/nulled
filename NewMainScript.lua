...
local customdir = (shared.nulledPrivate and "nulledprivate/" or "nulled/")
local betterisfile = function(file)
	local suc, res = pcall(function() return readfile(file) end)
	return suc and res ~= nil
end
local function GetURL(scripturl)
	if shared.nulledDeveloper then
		if not betterisfile("nulled/"..scripturl) then
			error("File not found : nulled/"..scripturl)
		end
		return readfile("nulled/"..scripturl)
	else
		local res = game:HttpGet("https://raw.githubusercontent.com/NBDMatr1x/nulled/main/"..scripturl, true)
...
local function checkassetversion()
	local req = requestfunc({
		Url = "https://raw.githubusercontent.com/NBDMatr1x/nulled/main/assetsversion.dat",
		Method = "GET"
	})
...
if not (getasset and requestfunc and queueteleport) then
	print("nulled not supported with your exploit.")
	return
end

if shared.nulledExecuted then
	error("nulled Already Injected")
	return
else
	shared.nulledExecuted = true
end

if isfolder(customdir:gsub("/", "")) == false then
	makefolder(customdir:gsub("/", ""))
end
if isfolder("nulled") == false then
	makefolder("nulled")
end
if not betterisfile("nulled/assetsversion.dat") then
	writefile("nulled/assetsversion.dat", "1")
end
...
if not betterisfile("nulled/language.dat") then
	writefile("nulled/language.dat", "en-us")
end
local assetver = checkassetversion()
if assetver and assetver > readfile("nulled/assetsversion.dat") then
	if shared.nulledDeveloper == nil then
		if isfolder("nulled/assets") then
			if delfolder then
				delfolder("nulled/assets")
			end
		end
		writefile("nulled/assetsversion.dat", assetver)
	end
end
if isfolder("nulled/assets") == false then
	makefolder("nulled/assets")
end

local GuiLibrary = loadstring(GetURL("NewGuiLibrary.lua"))()
local translations = {}--loadstring(GetURL("translations/"..GuiLibrary["Language"]..".nulledtranslation"))()
local translatedlogo = false--pcall(function() return GetURL("translations/"..GuiLibrary["Language"].."/nulledLogo1.png") end)

local checkpublicreponum = 0
local checkpublicrepo
checkpublicrepo = function(id)
	local suc, req = pcall(function() return requestfunc({
		Url = "https://raw.githubusercontent.com/NBDMatr1x/nulled/main/CustomModules/"..id..".lua",
		Method = "GET"
	}) end)
	if not suc then
...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/NBDMatr1x/nulled/main/"..path:gsub("nulled/assets", "assets"),
			Method = "GET"
		})
...
local Combat = GuiLibrary.CreateWindow({
	["Name"] = "Combat", 
	["Icon"] = "nulled/assets/CombatIcon.png", 
	["IconSize"] = 15
})
local Blatant = GuiLibrary.CreateWindow({
	["Name"] = "Blatant", 
	["Icon"] = "nulled/assets/BlatantIcon.png", 
	["IconSize"] = 16
})
local Render = GuiLibrary.CreateWindow({
	["Name"] = "Render", 
	["Icon"] = "nulled/assets/RenderIcon.png", 
	["IconSize"] = 17
})
local Utility = GuiLibrary.CreateWindow({
	["Name"] = "Utility", 
	["Icon"] = "nulled/assets/UtilityIcon.png", 
	["IconSize"] = 17
})
local World = GuiLibrary.CreateWindow({
	["Name"] = "World", 
	["Icon"] = "nulled/assets/WorldIcon.png", 
	["IconSize"] = 16
})
local Friends = GuiLibrary.CreateWindow2({
	["Name"] = "Friends", 
	["Icon"] = "nulled/assets/FriendsIcon.png", 
	["IconSize"] = 17
})
local Profiles = GuiLibrary.CreateWindow2({
	["Name"] = "Profiles", 
	["Icon"] = "nulled/assets/ProfilesIcon.png", 
	["IconSize"] = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
spawn(function()
	...
	until (not shared.nulledExecuted)
end)
...
ProfilesTextList = Profiles.CreateTextList({
	...
	["RemoveFunction"] = function(num, obj) 
		if obj ~= "default" and obj ~= GuiLibrary["CurrentProfile"] then 
			pcall(function() delfile(customdir.."Profiles/"..obj..(shared.CustomSavenulled or game.PlaceId)..".nulledprofile.txt") end)
	...
	["CustomFunction"] = function(obj, profilename) 
		...
		bindimg.Image = getcustomassetfunc("nulled/assets/KeybindIcon.png")
		...
		bindbkg.MouseEnter:connect(function() 
			bindimg.Image = getcustomassetfunc("nulled/assets/PencilIcon.png") 
			bindimg.Visible = true
			bindtext.Visible = false
			bindbkg.Size = UDim2.new(0, 20, 0, 21)
			bindbkg.Position = UDim2.new(1, -50, 0, 6)
...
OnlineProfilesButtonImage.Image = getcustomassetfunc("nulled/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = getcustomassetfunc("nulled/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = getcustomassetfunc("nulled/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = getcustomassetfunc("nulled/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:connect(function()
	...
	if profilesloaded == false then
		...
		local success, result = pcall(function()
			return game:GetService("HttpService"):JSONDecode((shared.nulledDeveloper and readfile("nulled/OnlineProfiles.nulledonline") or game:HttpGet("https://raw.githubusercontent.com/NBDMatr1x/nulled/main/OnlineProfiles.nulledonline", true)))
		end)
		onlineprofiles = (success and result or {})
		for i2,v2 in pairs(onlineprofiles) do
			if tostring(v2["ProfileGame"]) == tostring(shared.CustomSavenulled or game.PlaceId) then
				...
				profiledownload.MouseButton1Click:connect(function()
					writefile(customdir.."Profiles/"..v2["ProfileName"]..tostring(game.PlaceId)..".nulledprofile.txt", (shared.nulledDeveloper and readfile("nulled/OnlineProfiles/"..v2["OnlineProfileName"]) or game:HttpGet("https://raw.githubusercontent.com/NBDMatr1x/nulled/main/OnlineProfiles/"..v2["OnlineProfileName"], true)))
...
GUI.CreateDivider()
---GUI.CreateCustomButton("Favorites", "nulled/assets/FavoritesListIcon.png", UDim2.new(0, 17, 0, 14), function() end, function() end)
--GUI.CreateCustomButton("Text GUIVertical", "nulled/assets/TextGUIIcon3.png", UDim2.new(1, -56, 0, 15), function() end, function() end)
local TextGui = GuiLibrary.CreateCustomWindow({
	["Name"] = "Text GUI", 
	["Icon"] = "nulled/assets/TextGUIIcon1.png", 
	["IconSize"] = 21
})
local TextGuiCircleObject = {["CircleList"] = {}}
--GUI.CreateCustomButton("Text GUI", "nulled/assets/TextGUIIcon2.png", UDim2.new(1, -23, 0, 15), function() TextGui.SetVisible(true) end, function() TextGui.SetVisible(false) end, "OptionsButton")
GUI.CreateCustomToggle({
	["Name"] = "Text GUI", 
	["Icon"] = "nulled/assets/TextGUIIcon3.png",
...
onething.Image = getcustomassetfunc(translatedlogo and "nulled/translations/"..GuiLibrary["Language"].."/nulledLogo3.png" or "nulled/assets/nulledLogo3.png")
...
onething2.Image = getcustomassetfunc("nulled/assets/nulledLogo4.png")
...
TextGui.CreateToggle({
	...
	["HoverText"] = "Renders a nulled watermark"
})
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	["Name"] = "Target Info",
	["Icon"] = "nulled/assets/TargetInfoIcon1.png",
	["IconSize"] = 16
})
...
healthbarbkgshadow.Image = getcustomassetfunc("nulled/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	["Name"] = "Target Info", 
	["Icon"] = "nulled/assets/TargetInfoIcon2.png", 
...
local teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started and not shared.nulledIndependent then
		local teleportstr = 'shared.nulledSwitchServers = true if shared.nulledDeveloper then loadstring(readfile("nulled/NewMainScript.lua"))() else loadstring(game:HttpGet("https://raw.githubusercontent.com/NBDMatr1x/nulled/main/NewMainScript.lua", true))() end'
		if shared.nulledDeveloper then
			teleportstr = 'shared.nulledDeveloper = true '..teleportstr
		end
		if shared.nulledPrivate then
			teleportstr = 'shared.nulledPrivate = true '..teleportstr
		end
		if shared.nulledCustomProfile then 
			teleportstr = "shared.nulledCustomProfile = '"..shared.nulledCustomProfile.."'"..teleportstr
		end
		GuiLibrary["SaveSettings"]()
...
GuiLibrary["SelfDestruct"] = function()
	...
	shared.nulledExecuted = nil
	shared.nulledPrivate = nil
	shared.nulledFullyLoaded = nil
	shared.nulledSwitchServers = nil
	shared.GuiLibrary = nil
	shared.nulledIndependent = nil
	shared.nulledManualLoad = nil
	shared.CustomSavenulled = nil
...
GeneralSettings.CreateButton2({
	...
	["Function"] = function()
		local nulledprivate = shared.nulledPrivate
		GuiLibrary["SelfDestruct"]()
		delfile(customdir.."Profiles/"..(GuiLibrary["CurrentProfile"] == "default" and "" or GuiLibrary["CurrentProfile"])..game.PlaceId..".nulledprofile.txt")
		shared.nulledSwitchServers = true
		shared.nulledOpenGui = true
		shared.nulledPrivate = nulledprivate
...
if shared.nulledIndependent then
	spawn(function()
		repeat task.wait() until shared.nulledManualLoad
		GuiLibrary["LoadSettings"](shared.nulledCustomProfile)
		...
		GuiLibrary["UpdateUI"]()
		UpdateHud()
		if not shared.nulledSwitchServers then
			if blatantmode["Enabled"] then
				pcall(function()
					local frame = GuiLibrary["CreateNotification"]("Blatant Enabled", "nulled is now in Blatant Mode.", 5.5, "assets/WarningNotification.png")
		...
		if shared.nulledOpenGui then
			GuiLibrary["MainGui"].ScaledGui.ClickGui.Visible = true
			game:GetService("RunService"):SetRobloxGuiFocused(GuiLibrary["MainBlur"].Size ~= 0) 
			shared.nulledOpenGui = nil
		end

		coroutine.resume(selfdestructsave)
	end)
	shared.nulledFullyLoaded = true
	return GuiLibrary
else
	loadstring(GetURL("AnyGame.lua"))()
	if betterisfile("nulled/CustomModules/"..game.PlaceId..".lua") then
		loadstring(readfile("nulled/CustomModules/"..game.PlaceId..".lua"))()
	else
	...
	if shared.nulledPrivate then
		if pcall(function() readfile("nulledprivate/CustomModules/"..game.PlaceId..".lua") end) then
			loadstring(readfile("nulledprivate/CustomModules/"..game.PlaceId..".lua"))()
		end	
	end
	GuiLibrary["LoadSettings"](shared.nulledCustomProfile)
	local profiles = {}
